...
modo embutido 7B FF 03 <mode> FF FF FF FF BF          7B 03 <mode> FF FF FF FF FF BF
modo DIY      7B FF 13 <mode> FF FF FF FF BF          7B 13 <mode> FF FF FF FF FF BF
...
⚠️ **`0x03` e `0x13` se confundem fácil.** `0x03` seleciona modo **embutido**, da
tabela de 211 entradas. `0x13` seleciona os padrões **DIY** do usuário. Mandar id
embutido para o `0x13` não faz nada.

**O LEDCAR-01 tem frame de power próprio.** O app ramifica pelo nome *antes* do
envelope:
