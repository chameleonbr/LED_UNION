...
export const fff0: Driver = {
  ...
  /** This family separates enabling the mic from picking its EQ profile. */
  soundEnable: (on) => f(0x7e, 0x04, 0x07, on ? 0x01 : 0x00, 0xff, 0xff, 0xff, 0x00, 0xef),

  soundSensitivity: (v) =>
    f(0x7e, 0x04, 0x06, pct(v), 0xff, 0xff, 0xff, 0x00, 0xef),

  soundMode(mode, name) {
    // The EQ profile is offset into the 0x80 range, like this family's effect ids.
    const m = (byte(mode) | 0x80) & 0xff
    return isMelk(name)
      ? f(0x7e, 0x07, 0x03, m, 0x04, 0xff, 0xff, 0x00, 0xef)
      : f(0x7e, 0x05, 0x03, m, 0x04, 0xff, 0xff, 0x00, 0xef)
  },
